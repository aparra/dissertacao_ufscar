...
Após detalharmos os componentes de Treplica e como eles estão relacionados, nesta seção
iremos focar nossa discussão nas alterações propostas para expandir biblioteca. Até o
momento, todos os conceitos foram apresentados para embasar as novas funcionalidades:
...
motivo nobre: expansão do aglomerado. No entanto, segundo \citeonline{lamport10}, toda
operação para adição de réplica em Paxos deve ser precedida de uma operação não trivial de
reconfiguração.

Independente da razão para aplicação de uma transferência de estado, essa operação não
deve gerar grande impacto no processamento do grupo. Tendo em vista a garantia de
consistência, a transferência de estado é implementada como uma tarefa síncrona. As
