...
Treplica \cite{vieira08a, vieira-tr10b} é uma biblioteca de replicação em Java que permite
construir aplicações distribuídas que atendam ao modelo de replicação ativa. A função
principal desempenhada por Treplica é gerenciar o estado da aplicação, executando as
requisições dos clientes através de uma abstração de uma \emph{máquina de estados
replicada}. A ordenação das requisições antes da sua efetiva execução é feita por uma
abstração de \emph{fila persistente assíncrona}, implementada atraves do algoritmo Paxos.

Arquiteturalmente a biblioteca estah dividida em dois grandes grupos: (1) componentes de
suporte e (2) componentes de Paxos. Essas duas macro estruturas serao apresentadas com
detalhes a seguir, o foco dessa secao eh apresentar somente os componentes de Treplica que
serao utilizados pelas alteracoes propostas na API, a descricao completa da biblioteca
pode ser encontrada em \cite{vieira-tr10b}.
...
Este componente utiliza memória persistente usando o componente \classname{Ledger}, lida
com a passagem de mensagens usando o componente de transporte e lida também com a fila de
