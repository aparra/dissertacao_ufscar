...
iterações entre réplicas e os bloqueios de estado para garantia de consistência.
Apresentaremos os detalhes dessa implementação nessa seção. Para maior clareza de
exposição, quando necessário, chamaremos as réplicas que recebem o estado como
\emph{réplicas receptoras} e réplicas que transferem seu estado como \emph{réplicas
doadoras}.
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=11cm]{conteudo/capitulos/figuras/inclusao_replica.eps}
...
Para incluir uma nova réplica ou para preencher grandes lacunas precisamos que a operação
de transferência de estado sejam produtivas. Ou seja, essa operação deve ter o mínimo de
impacto no processamento do grupo, e no caso de inclusão de réplica, deve trazer aumento
de capacidade de processamento. Por questões de garantia de consistência, a transferência
de estado é implementada como uma tarefa síncrona. Supomos, as seguintes premissas para
...
\subsection{Funcionamento do protocolo}
