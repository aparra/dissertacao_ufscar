...
A classe \classname{PaxosReadonlyQueue} possui o mesmo objetivo da classe
\classname{PaxosPersistenteQueue}, já existente na biblioteca Treplica: fornecer uma fila
que será utilizada pelo protocolo Paxos. No entanto, as operações executadas por
\classname{PaxosReadonlyQueue} são bem difrerentes:

Com relação ao processamento das mensagens processadas por essa fila, supomos as seguintes
propriedades:
...
O principal objetivo dessa classe é processar mensagens não relacionados com a decisão de
consenso, dessa forma oferece um poder de manobra para direcionar

Essa classe utiliza o componente \classname{WeakSecretary}
